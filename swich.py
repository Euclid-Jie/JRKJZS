...
sheet_name = "测试1"
##表头
value_title = [["专利名","阶段","申请号", " 申请日", "发明人","公开号","公开日","公司名","专利类型"]]
...
for i in range(0,numbers):
	if len(invents[i].find_all('td'))==2:
		try:
			value = [];
			...
			print("第"+str(i)+"条数据错误")
			create_str_to_txt(str(invents[i])+'\n')
	else:
		tds = invents[i].find_all('td')
		td1 = tds[1]
		td2 = tds[2]
		try:
			value = [];
			inventname = td1.div.a;
			value.append(inventname.get_text())
			stage = td1.div.div.span
			value.append(stage.get_text())
			div2 = td1.div.find("div","relate-info")
			spans = div2.find_all("span","val")
			value.append([spans[0].string])
